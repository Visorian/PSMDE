...
Describe "Get-MdeMachineInvestigationPackage" {
  ...
  It 'Should correctly create the request uri' {
    InModuleScope PSMDE {
      ...
      $id = '12345'
      $comment = 'Comment'
      Get-MdeMachineInvestigationPackage -id $id -comment $comment | Should -Be "https://api.securitycenter.microsoft.com/api/machines/$id/collectInvestigationPackage"
    }
  }
}