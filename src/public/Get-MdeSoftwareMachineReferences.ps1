...
.OUTPUTS
  PSCustomObject. The Get-MdeSoftwareMachineReferences function returns a list of or a single PSCustomObject containing the parsed MDE machine object.

.EXAMPLE
  Get-MdeSoftwareMachineReferences -id 'microsoft-_-edge'

.ROLE
  @(@{permission = 'Software.Read.All'; permissionType = 'Application'}, @{permission = 'Software.Read'; permissionType = 'Delegated'})
#>

function Get-MdeSoftwareMachineReferences {
  [CmdletBinding()]
  param (
  ...
  Process {
    return Invoke-AzureRequest -Uri "https://api.securitycenter.microsoft.com/api/Software/$id/machineReferences"
  }
  End {}
}